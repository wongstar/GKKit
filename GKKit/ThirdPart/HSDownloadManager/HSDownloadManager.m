...
- (void)download:(NSString *)url progress:(void (^)(NSInteger, NSInteger, CGFloat))progressBlock state:(void (^)(DownloadState))stateBlock
{
    ...
    NSLog(@"the url is %@",HSFileName(url));
...
-(void)pauseAllTask{
    NSArray *ar=[self.tasks allValues];
    for(int i=0;i<ar.count;i++){
        NSURLSessionDataTask *task=[ar objectAtIndex:i];
        [task cancel];
...
-(BOOL)isExitFile:(NSString *)url{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    return [fileManager fileExistsAtPath:HSFileFullpath(url)];
}
